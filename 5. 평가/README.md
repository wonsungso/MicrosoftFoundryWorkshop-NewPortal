...
4. `새 데이터 세트 업로드` 또는 **`직접 입력`** 옵션을 선택합니다.
5. 위의 JSONL 샘플 데이터를 복사해 입력 필드에 붙여넣습니다 (한 줄에 하나의 JSON 객체).
6. 데이터 미리보기를 확인하고 다음 단계로 이동합니다.
...
4. **필드 매핑** 화면에서 데이터 구조를 다음과 같이 연결합니다.

   - **쿼리**: `query` (사용자의 펀드 추천 요청)
   - **응답**: `response` (워크플로우가 생성한 추천)
   - **컨텍스트**: `context` (사용자 프로필 정보)

5. 평가 이름은 `CreditToFund_eval_v1` 또는 `펀드추천_테스트_v1`처럼 입력합니다.
6. 마지막에 `제출`을 클릭합니다.
